...
sequenceDiagram
    ...
    P->>P: debounce 150ms (coalesces rapid repeated seeks)
...
- **In-place quality switching** — `switchToAsset()` swaps `audio.src` to a different rendition, restores `currentTime` once `loadedmetadata` fires, and resumes playback if it was playing. Downgrade triggers after two stalls within 20 seconds (seek-induced buffering is excluded from that count); upgrade is checked opportunistically every 25 seconds while playing. A monotonic `switchRequestRef` generation guard ensures only the most recently requested switch can act — an older switch whose `loadedmetadata` resolves after a newer one has already taken over is a no-op instead of racing it. See [the quality-switch race issue writeup](issues/client-issues.md#rapid-seeking-triggered-cascading-quality-downgrades-and-a-switchtoasset-race).
- **Native range-based seeking** — seeking computes the clamped target time immediately (for responsive UI) but debounces the actual `audio.currentTime` write by 150ms, coalescing rapid repeated seeks (slider dragging, repeated arrow-key presses) into a single write instead of a burst of aborted/restarted `Range` requests, relying on `+faststart` encoding so the media engine can jump to arbitrary offsets without downloading everything before them. If playback still fails to (re)start, `resetStream()` detaches `audio.src` so the next Play press probes and attaches fresh rather than retrying the same broken resource indefinitely. See [ADR-0004](adr/0004-native-range-based-seeking.md), [the forward-seek issue writeup](issues/client-issues.md#infinite-loading-state-on-forward-seek), and [the rapid-seek stuck-playback writeup](issues/client-issues.md#rapid-seeking-could-wedge-playback-until-a-page-refresh).
